 ...
 * `avatarUrl` are nullable rather than nullable value objects — there is
 * no invalid non-empty state to guard against once either is `null`, so
 * there is nothing for a wrapper to validate in that case.
 */
