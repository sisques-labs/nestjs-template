...
| Users | `src/contexts/users/` | Self-service `GET`/`PATCH /api/users/me` — a tenant-scoped profile (`displayName`, `email`) lazily created from the verified principal, the first real consumer of `TenantScopedRepository`. Opt-in (registers only when `IDENTITY_PROVIDER && TENANCY_ENABLED`). See `src/contexts/users/README.md` |
